...
int8_t get_worker_states() {
    ...
    int8_t workerStates[4] = {genWorker1Reg[3], genWorker2Reg[3], genWorker3Reg[3], genWorker4Reg[3]};
    int8_t maxValue = 0;
    for (int i = 0; i < 4; i++) {
        if (workerStates[i] > maxValue) {
            maxValue = workerStates[i];
        }
    }
    return maxValue;
}
...
int64_t get_current_seed() {
    int32_t worldSeedLow = XBram_ReadReg(XPAR_BRAM_0_BASEADDR, 0);
    int32_t worldSeedHigh = XBram_ReadReg(XPAR_BRAM_0_BASEADDR+4, 0);
    int64_t worldSeed = ((int64_t)worldSeedHigh << 32) | worldSeedLow;
    printf("Current World Seed: %lld\n\r", worldSeed);
    return worldSeed;
}
...
int32_t init_workers() {
    ...
    int reset_result4 = reset_worker(genWorker4Reg, genWorker4BaseAddress, 4);
